...
os.chdir(r'/gss1/home/gaohao/Oryza_sativa_methylation_picture/A_2.methylation')#修改工作目录

inputfile1 = open("all.gff3")
inputfile2 = open("A_2.sortBYhao.CpG_report.txt") #修改要输入的文件名

def get_area_me_mean(all_area_num,gb_area_num,gud_area_length,c_type):#总共要划分为多少个区段（偶整数），基因内部划分多少个区段（偶整数），基因上下游取多少长度（bp）,基因上下游取的长度除以上下游的区段数目要是个整数,甲基化的类型（"CG"/"CHH"/"CHG"）
    ID = []
    gene_start = []
    gene_end = []
    strand = []
...
#Tigr7_genes_start_end.to_csv(path_or_buf = r"/gss1/home/gaohao/Oryza_sativa_methylation_picture/A_1.methylation/Tigr7_genes_start_end.csv")
    ...
    _TSS_TES_.to_csv(path_or_buf  = r"/gss1/home/gaohao/Oryza_sativa_methylation_picture/A_2.methylation/_TSS_TES_.csv")
    ...
    data1 = {"chromosome":chromosome,"position":position,"count_methylated":count_methylated,"count_unmethylated":count_unmethylated,"C_context":C_context}

    A_1_CpG = pd.DataFrame(data1)

    A_1_CpG.to_csv(path_or_buf  = r"/gss1/home/gaohao/Oryza_sativa_methylation_picture/A_2.methylation/CpG.csv")

    inputfile3 = open("CpG.csv")#每列依次为：行号,chromosome,position,count_methylated,count_unmethylated,C_context
...
get_area_me_mean(80,40,1000,'CG')
